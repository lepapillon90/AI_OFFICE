...
- 구성원 초대 (`docs/PHASE4_SUPABASE.md`의 "인사관리자 초대" 절)
  ...
  - `invites` 테이블 미생성 시 초대 시도 시 에러 메시지로 안내 (앱 자체는 깨지지 않음) — 화면에서 에러 메시지 노출까지 확인, 실제 초대 수락 흐름은 SQL 실행 후 재검증 필요

## 다음 작업

1. `docs/PHASE4_SUPABASE.md`의 `invites` 테이블 SQL을 Supabase SQL Editor에서 실행 (구성원 초대 기능에 필요)
2. `docs/PHASE5_MULTIPLAYER.md`의 `messages` 테이블 SQL을 Supabase SQL Editor에서 실행 (채팅 기록 영속화에 필요)
3. 위 두 SQL 실행 후 초대 수락(초대받은 아이디로 로그인 → 자동 합류)과 공간 채팅의 실시간 수신을 실제 브라우저 두 개로 재검증
4. AI 직원 연결 (실제 제공자 API로 NPC 상태 갱신, Phase 6)

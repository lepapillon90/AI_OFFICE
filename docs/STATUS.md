...
1. `docs/PHASE5_MULTIPLAYER.md`의 `messages` 테이블 SQL과 `docs/PHASE6_AI_EMPLOYEES.md`의 컬럼 추가 SQL을 아직 안 하셨다면 Supabase SQL Editor에서 실행 (채팅 기록 영속화·귓속말/AI 구분에 필요) — 이미 실행하셨다면 완료 상태
2. **미검증**: 다른 사용자가 보낸 "공간 채팅"(공개) 메시지의 실시간 수신을 실제 브라우저 두 개로 검증 — 이번 세션엔 내가 보내는 방향(귓속말 발신, presence)만 재확인됐고, 다른 사람이 보낸 공개 메시지를 내가 실시간으로 받는 경로는 아직 재검증 안 됨

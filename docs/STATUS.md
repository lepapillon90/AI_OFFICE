...
- **Phase 8 — 회사 컴퓨터 원격 명령(터미널 열기 · 폴더 만들기, 직원별 다중 컴퓨터)** (`docs/PHASE8_REMOTE_AGENT.md`)
  ...
  - **화이트리스트 방식**: 채팅 문장이 임의의 셸 명령으로 그대로 실행되는 게 아니라, `RemoteCommandType`(`lib/data/remote_command.dart`)에 정의된 `open_terminal`/`create_folder`/`open_terminal_claude` 세 가지만 허용 — `OfficeGame._parseRemoteCommand`가 단순 키워드 매칭으로 문장을 그 중 하나로 해석하고, 해당 없는 문장은 아무 것도 실행하지 않고 "지원하지 않는 명령" 안내만 나감
  - **후속 반영 — Claude CLI 자동 실행**: "터미널 열어서 claude 실행해줘"처럼 "터미널"과 함께 "claude"(영문)/"클로드"가 언급되면 `open_terminal_claude`로 해석 — 터미널을 열면서 그 안에서 `claude` CLI 하나만(다른 임의 프로그램/명령은 여전히 불가) 곧바로 실행(`cmd /k claude`). 라이브 테스트 중 발견한 두 버그(로스터 체크박스 UI 미갱신, 에이전트의 한글 결과 UTF-8 인코딩 누락으로 인한 무한 재실행)도 이 과정에서 함께 수정
  ...
  - 자동 테스트(`test/remote_command_test.dart`) 12개로 명령 파싱(터미널/폴더 이름 추출/부가 표현 무시, claude CLI 인식)·화이트리스트 밖 명령 거절·핸들러 미설정 시 안내·빈 멘션은 디스패치 안 함·컴퓨터 연결된 직원은 machineKey=workstationId로 라우팅되고 LLM은 호출 안 됨·연결 안 된 직원은 "터미널/폴더" 단어가 있어도 평소처럼 LLM으로만 감을 검증 — 실제 컴퓨터에서 에이전트를 띄워 SQL 실행 후 라이브로 터미널이 열리고 폴더가 생기는지는 사용자 환경에서 확인 필요(서비스 롤 키가 없어 이 세션에서는 에이전트의 Supabase 왕복 자체는 라이브 검증 못함, 순수 로직/컴파일/테스트만 확인)
  - **후속 반영 — Windows 자동 시작**: `scripts/install_remote_agent_task.ps1`로 Windows 예약 작업(로그온 시 실행, 크래시 시 자동 재시작)에 등록 가능 — 비밀 키는 CLI 인자가 아니라 `bin/remote_agent.config.json`(`.gitignore` 대상, 예시는 `bin/remote_agent.config.example.json`)에서 읽어서, 예약 작업 자체엔 비밀이 저장되지 않음. 직접 `dart run bin/remote_agent.dart`(인자 없이)로 실행해 config 파일 인식을 확인
  - **후속 반영 — 서버 잠금(게임 속 서버기계)**: 사용자 요청으로 2층에 클릭 가능한 "서버기계" 오브젝트(`ServerMachineInteraction`, 그림 에셋 없이 도형+텍스트로 그림) 추가 — `@서버`(기본 컴퓨터) 명령만 이 비밀번호로 잠기고, 직원별 연결 컴퓨터는 옵트인 체크박스로만 제어되어 영향 없음. 비밀번호는 `company_integrations.server_password`에 저장하고 `has_server_password`/`verify_server_password` 두 RPC(boolean만 반환)로만 확인 — 관리자 화면의 입력창은 항상 비어서 시작(write-only 설계, 기존 값을 다시 보여주지 않음). 비밀번호 미설정 시 기본값은 "잠금 없음"(옵트인, Slack 연동과 같은 패턴). 명시적으로 "실제 보안 경계가 아니라 게임 속 잠금장치"로 문서화 — 진짜 경계는 여전히 명령 화이트리스트와 직원별 옵트인. **라이브 검증 중 발견한 한계**: `revoke select (server_password) ...`로 클라이언트가 원본 값을 못 읽게 막으려 했으나, 라이브 테스트에서 REVOKE가 실제로는 안 먹혀서 `.select('server_password')`로 직접 조회하면 여전히 평문이 보임(RPC 두 개는 정상 동작) — write-only라는 문서 설명이 100% 정확하지 않음, 후속으로 원인 파악 필요. 자동 테스트(`test/server_lock_test.dart`) 10개(기존 5개 + 전원 스위치 5개)로 검증
  - **후속 반영 — 서버 실행/종료(전원 스위치)**: 사용자 요청으로 잠금 해제 후 "서버 실행"/"서버 종료" 버튼 추가 — 에이전트 프로세스 자체는 예약 작업으로 항상 켜져 있을 수 있지만, `server_control.running`이 true일 때만 실제로 `pending` 명령을 처리하고, false면 곧바로 `failed`로 "서버가 꺼져 있습니다" 안내만 돌려줌(브라우저가 실제 프로세스를 못 켜는 근본 제약은 여전해서, "실행" 버튼은 "이미 떠 있는 에이전트에게 처리를 허락"하는 스위치일 뿐임을 문서에 명시). 비밀번호와 달리 보안 경계가 아니라 회사 전체가 공유하는 전원 버튼이라 잠금 해제한 구성원 누구나 켜고 끌 수 있음(RLS도 구성원 전체 read/write) — 실시간 동기화는 안 해서 다른 사람이 바꾼 상태는 팝업을 다시 열어야 반영(이연). 라이브 브라우저에서 서버기계 클릭 → 다이얼로그 오픈까지는 확인했으나, 전원 버튼 자체의 라이브 클릭 테스트는 이 세션의 플레이어 이동 자동화 제약으로 못함 — 단위 테스트(`fetchServerRunning`/`updateServerRunning` 관련 5개)로만 검증
  - 이연: 화이트리스트 확장(파일 목록/삭제/스크립트 실행 등), macOS/Linux 자동 시작, 전원 스위치 상태 실시간 동기화, `server_password` REVOKE가 안 먹히는 문제 원인 파악

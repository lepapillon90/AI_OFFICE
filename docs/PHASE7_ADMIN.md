...
- ~~구성원 강퇴(회사에서 제거)는 이번 범위 아님~~ — Phase 8 후속 반영에서 완료(`docs/STATUS.md` 참고): 관리자 화면 구성원 목록에 "구성원 제거" 버튼 추가(대표 전용, 확인 다이얼로그 거침). 계정 자체는 삭제되지 않고 `company_members` 행만 제거됨 — 기존 `owner_manage_members` RLS 정책이 이미 DELETE까지 `for all`로 포함하고 있어 새 정책 불필요
- ~~감사 기록의 필터/검색(행위자별, 기간별 등)은 이번 범위 아님~~ — Phase 8 후속 반영에서 완료: 활동 기록 패널에 유형별 필터 칩(직원/AI 명령/보드/회의/구성원)과 메시지·행위자 텍스트 검색 추가. 이미 불러온 최근 50건 안에서 클라이언트 단에서 필터링(서버 쪽 페이지네이션/검색은 아님)
